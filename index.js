...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Opa! @${num.split('@')[0]}\nMembro novo mama o ademiro😳 *${mdata.subject}*`
				...
				teks = `Ja foi tardeKKKKKK @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Pera um pouco😔✋',
				success: 'Pronto, agr me deve uma mamada😳️',
				error: {
					stick: '❌ Falha, ocorreu um erro ao converter a imagem em um adesivo ❌',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					group: '⚠️ Este comando só pode ser usado em grupos idiota!',
					ownerG: 'Este comando só pode ser usado pelo ademiro🙄',
					ownerB: '⚠️ Este comando só pode ser usado pelo dono do bot!',
					admin: 'Este comando só pode ser usado pelo ademiro🙄',
					Badmin: 'Este comando só pode ser usado quando o bot se torna ademiro🙄'
				}
			}
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nome do bot* : ${me.name}\n*Número do bot* : @${me.jid.split('@')[0]}\n*Prefixo* : ${prefix}\n*Contatos Bloqueados* : ${blocked.length}\n*Tempo de Atividade* : ${kyun(uptime)}`
			    ...
			    case 'dono':
					...
					buffer = await getBuffer(`pin.it/cnzkqG2`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Nome:* Artukkj\n*Zap:* wa.me/559882832355\n*Me chama ai n tenho amigos😔✋*'})
					break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
